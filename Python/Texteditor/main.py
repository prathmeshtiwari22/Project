...
def insert_image():
        ...
        filename = filedialog.askopenfilename(filetypes=[("Image Files", "*.gif;*.png;*.jpg;*.jpeg")])
        if filename and os.path.exists(filename):
            try:
                # Load the image using PhotoImage
                image = PhotoImage(file="C:/Users/santo/OneDrive/Desktop/COLLEGE/random stuffs/672077-top-texas-tech-university-wallpapers-1920x1200-ipad.jpg")
                # Set focus on the text widget
                # textarea.focus_set()
                # Insert the image at the current cursor position
                textarea.image_create(END, image=image)
                print("succcess")
...
#insert=PhotoImage(file='insertimage.png')
#insertimageButton=Button(tool_bar,image=insert,command=insert_image)
...
texttospeech=Menu(menubar,tearoff=False)
menubar.add_cascade(label='Text to Speech',menu=texttospeech)
